...
class GMRC(TMRC):
    ...
    body_ratio = 2 / 3          # The body ratio of a module, participating in collision
    radius_ratio = 1 / 10       # The body radius ratio for a module
    max_num_segments = 11       # Number of body segments when bending 180 degrees
    ...
    def __init__(self, tmrc, bending_angles = None, grasping_angles = None, 
        ...
        self.c = len(self.real_cycles)  # Number of cycle basis (cyclomatic number)

        self.is_grip_w = self.get_is_grip_w()

        self.gripper2ht = self.get_gripper2ht()
    ...
    # Get a grip cycle formatted as: [*module, grip, module, grip, ..., grip, *module]
    ...
    # 0: head; 1: tail
    def get_gripper2ht(self):
        gripper2ht = [0] * len(self.grippers)
        for i in range(len(self.grippers)):
            gripper2ht[i] = self.gripper2module[i] % 2
        return gripper2ht
    ...
    def get_module_ht_loop(self, real_cycle):
        ...
        for i in range(len(module_ht_loop)):
            if self.gripper2ht[real_cycle[3 * i]] == 0:     # If head to tail
                module_ht_loop[i] = 1
            else:
                module_ht_loop[i] = -1
    ...
    # Get the gripper of the module in grip
    def get_gripper(self, module, grip):
        # NOTE: it is knonw that grip g has grippers 3 * g, 3 * g + 1 and 3 * g + 2
        # NOTE: it is also knonw that a module can not have both grippers in a grip
        gripper = self.module2gripper[0][module]
        if 3 * grip <= gripper and gripper <= 3 * grip + 2:
            return gripper
        else:
    ...
    def get_loop_dock_error(self, x, is_print = False):
        ...
        for i in range(len(self.ba_xi_loops)):
            ...
            beta_sign = np.sign(betas)
            gammas = x[self.ga_xi_loops[i]] * self.gas_loops[i]

            a = np.sinc(betas / np.pi)
            c = np.sinc(betas / 2 / np.pi)
            b = np.sqrt(c ** 2 - a ** 2) * beta_sign
    ...
    # Get all neibor grippers for a gripper:
    ...
    def update_module_geometry(self, mi, sp, sa, ht):
        ...
        a = np.sinc(ar / np.pi)
        c = np.sinc(ar / np.pi / 2)
        b = np.sign(ar) * np.sqrt(c ** 2 - a ** 2)
        ep = (sp[0] + a * np.cos(sa) - b * np.sin(sa),  # End position
              sp[1] + a * np.sin(sa) + b * np.cos(sa))
        ea = sa + ar                                    # End angle
        ...
        for i in range(2):
            if self.module2gripper[i][mi] >= 0: # If it participates in a grip
                ...
                gns = self.get_gripper_neighbors(gripper)   # Gripper Neighbors
                for gn in gns:
    ...
    def get_module_collider(self, geometry):
        ...
        alpha = geometry[1]
        beta = geometry[2]
        number_segments = int(np.ceil(np.abs(beta) / np.pi * GMRC.max_num_segments))
        number_segments = max(1, number_segments)
        start_beta = beta * (1 - GMRC.body_ratio) / 2
        end_beta = beta * (1 - (1 - GMRC.body_ratio) / 2)
        betas = np.linspace(start_beta, end_beta, number_segments + 1)
        
        a = np.sinc(betas / np.pi) * betas / beta
        c = np.sinc(betas / np.pi / 2) * betas / beta
        b = np.sign(beta) * np.sqrt(c ** 2 - a ** 2)
        axis_x = sp[0] + a * np.cos(alpha) - b * np.sin(alpha)
        axis_y = sp[1] + a * np.sin(alpha) + b * np.cos(alpha)

        radius_ang = alpha + betas
        inner_x = axis_x + GMRC.radius_ratio * np.sin(radius_ang)
        inner_y = axis_y - GMRC.radius_ratio * np.cos(radius_ang)
        outer_x = axis_x - GMRC.radius_ratio * np.sin(radius_ang)
        outer_y = axis_y + GMRC.radius_ratio * np.cos(radius_ang)
        
        shell = np.zeros((2 * (number_segments + 1) + 1, 2))
        shell[0 : number_segments + 1, 0] = inner_x
        shell[number_segments + 1 : 2 * (number_segments + 1), 0] = outer_x[::-1]
        shell[2 * (number_segments + 1), 0] = inner_x[0]
        shell[0 : number_segments + 1, 1] = inner_y
        shell[number_segments + 1 : 2 * (number_segments + 1), 1] = outer_y[::-1]
        shell[2 * (number_segments + 1), 1] = inner_y[0]
    ...
    def draw_module(ax, sp, sa, ar, g1n, mn, g2n, pn = 11):
        angs = np.linspace(0, ar, pn)
        a = np.sinc(angs / np.pi) * angs / ar
        c = np.sinc(angs / np.pi / 2) * angs / ar
        b = np.sign(ar) * np.sqrt(c ** 2 - a ** 2)

        x = np.ones(len(angs)) * sp[0]
        y = np.ones(len(angs)) * sp[1]
        
        x = x + a * np.cos(sa) - b * np.sin(sa)
        y = y + a * np.sin(sa) + b * np.cos(sa)
        
        ax.plot(x, y, '-k')

        angs = np.array([ar / 4, ar / 2, 3 * ar / 4])
        a = np.sin(angs) / ar
        b = (1 - np.cos(angs)) / ar
        ...
        ax.text(x[0], y[0], g1n, bbox=dict(facecolor='red', alpha=0.5))
        ax.text(x[1], y[1], mn, bbox=dict(facecolor='blue', alpha=0.5))
        ax.text(x[2], y[2], g2n, bbox=dict(facecolor='red', alpha=0.5))